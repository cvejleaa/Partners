...
class GameController extends StateNotifier<GameState> {
  ...
  void startGame(List<PlayerSetup> setups,
    ...
    // som før). [cardRules] er de FÆRDIGT opløste regler for spillet — kalderen
    // (setup_screen) har allerede kørt effectiveCardRules(variant, live,
    // stored). Der opløses BEVIDST ikke igen her: én resolver, ét sted —
    // ellers ville kode-seedet (fx Hopsakortet) tavst blive genanvendt oven på
    // admins gemte valg.
    final BoardGeometry geom = variant.geometry;
    final CardRules resolvedRules = cardRules ?? CardRules.defaults();
    ...
    final int starter = pickStarter(variant, _rng);
...
class _PartnersAppState extends ConsumerState<PartnersApp>
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<PushMessage>>(pushMessageProvider, (prev, next) {
      ...
      if (msg == null) return;
      // Tur- og byttefase-beskeder om det spil, der er ÅBENT, springes over:
      // dem ser man på brættet. Om et andet spil (og invitationer) vises de.
      if (!showForegroundPush(msg, openOnlineGameCode.value)) return;
